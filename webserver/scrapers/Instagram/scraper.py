...
##code borrowed from https://github.com/rarcega/instagram-scraper/blob/master/instagram_scraper/app.py
...
class InstagramScraper(object):
    ...
    def __init__(self, **kwargs):
        default_attr = dict(username='', usernames=[], filename=None,
                            ...
                            quiet=False, maximum=0, media_metadata=False, profile_metadata=False, latest=True,
        ...
        try:
            ...
            print("Check is valid json type.")
            raise
    ...
    def get_dst_dir(self, username):
        ...
        if False:
            ...
            print('last_scraped_filemtime!!!:',self.last_scraped_filemtime)
        else:
            self.last_scraped_filemtime=0
    ...
    def query_comments_gen(self, shortcode, end_cursor=''):
        ...
        if comments:
            try:
                ...
                print('Failed to query comments for shortcode ' + shortcode)
    ...
    def worker_wrapper(self, fn, *args, **kwargs):
        try:
            ...
            print("Exception in worker thread", exc_info=sys.exc_info())
            raise
            ...
            for value in self.usernames:
                ...
                for item in tqdm.tqdm(media_generator(value), desc='Searching {0} for posts'.format(value), unit=" media",
                    ...
                    if ((item['is_video'] is False and 'image' in self.media_types) or \
                        ...
                        ) and self.is_new_media(item,value):
            ...
            for username in self.usernames:
                ...
                if not user:
                    ...
                    user['edge_owner_to_timeline_media']['edges']:
                        print('User {0} is private'.format(username))
                ...
                try:
                    ...
                    print("Unable to scrape user - %s" % username)
        finally:
            self.quit = True
            self.logout()
    ...
    def get_profile_pic(self, dst, executor, future_to_item, user, username):
        ...
        if self.logged_in:
            ...
            try:
                ...
                print('Failed to get high resolution profile picture for {0}'.format(username))
    ...
    def get_profile_info(self, dst, username):
        ...
        print( 'Saving metadata general information on {0}.json'.format(username) )
    ...
    def get_stories(self, dst, executor, future_to_item, user, username):
        ...
        if self.logged_in and \
            ...
            for item in tqdm.tqdm(stories, desc='Searching {0} for stories'.format(username), unit=" media",
                                  disable=self.quiet):
                if self.story_has_selected_media_types(item) and self.is_new_media(item,username):
        ...
        for item in tqdm.tqdm(self.query_media_gen(user), desc='Searching {0} for posts'.format(username),
            ...
            if self.filter:
                if 'tags' in item:
                    ...
                    if self.has_selected_media_types(item) and self.is_new_media(item,username) and filtered:
                        item['username']=username
                        future = executor.submit(self.worker_wrapper, self.download, item, dst)
                        future_to_item[future] = item
                else:
            ...
            # --------------#
                ...
                if self.has_selected_media_types(item) and self.is_new_media(item,username):
                    item['username']=username
                    future = executor.submit(self.worker_wrapper, self.download, item, dst)
                    future_to_item[future] = item
    ...
    def query_media_gen(self, user, end_cursor=''):
        ...
        if media:
            try:
                while True:
                    for item in media:
                        if not self.is_new_media(item,user['username']):
                            return
                        yield item
                ...
                print('Failed to query media for user ' + user['username'])
                        ...
                        while(True):
                                ...
                                with self.session.get(url, cookies=self.cookies, headers=headers, stream=True, timeout=CONNECT_TIMEOUT) as response:
                                    ...
                                    if response.status_code == 206:
                                        ...
                                        if content_length is None:
                                            pass #print('No Content-Length in response, the file {0} may be partially downloaded'.format(base_name))
                                        else:
                            ...
                            # Please do not add os.remove here.
                                ...
                                if downloaded - downloaded_before > 0:
                                    ...
                                    print('Continue after exception {0} on {1}'.format(repr(e), media))
                                    retry = 0 # the next fail will be first in a row with no data
                                    continue
                                if retry < MAX_RETRIES:
                                    print('Retry after exception {0} on {1}'.format(repr(e), media))
                                    self.sleep(retry_delay)
                                    retry_delay = min( 2 * retry_delay, MAX_RETRY_DELAY )
                                    retry = retry + 1
                                    continue
    ...
    def is_new_media(self, item, username):
