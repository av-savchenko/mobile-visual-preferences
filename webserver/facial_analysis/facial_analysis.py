...
class FacialImageProcessing:
    ...
    def __init__(self, print_stat=False, mtcnn_detector=True, minsize = 32):
        ...
        if age_gender_one_model:
            model_files[os.path.join(models_path,'age_gender_ethnicity_224_deep-03-0.13-0.97-0.88.pb')]='' # 'age_gender_tf2_new-01-0.14-0.92.pb'
            #model_files['D:/src_code/DNN_models/my_tf/age_gender_tf2_new-01-0.14-0.92_fbn.pb']='' #age_gender_tf2_192-01-0.16-0.92
        else:
    ...
    def load_age_gender(self,sess,graph):
        ...
        ethnicity_out=None
        ethnicity_tensors=[n.name for n in graph.as_graph_def().node if 'ethnicity_pred/Softmax' in n.name]
        print(ethnicity_tensors)
        if len(ethnicity_tensors)>0:
            ethnicity_out=graph.get_tensor_by_name('ethnicity_pred/Softmax:0')
        
        print(age_out,gender_out,ethnicity_out,facial_features_out)
        ...
        def age_gender_fun(img):
            ...
            if ethnicity_out is None:
                age_preds,gender_preds,facial_features = sess.run([age_out,gender_out,facial_features_out], feed_dict={in_img: x})
                ethnicity_preds=None
            else:
                age_preds,gender_preds,ethnicity_preds,facial_features = sess.run([age_out,gender_out,ethnicity_out,facial_features_out], feed_dict={in_img: x})
                ethnicity_preds=ethnicity_preds[0]
            ...
            return res_age,gender_preds,ethnicity_preds,facial_features
    ...
    def process_image(self,draw,face_bboxes=None):
        ...
        for b in bounding_boxes:
            ...
            if x2>x1 and y2>y1:
                ...
                w,h=x2-x1,y2-y1
                dw,dh=0,0 #10,10 #max(w//8,10),max(h//8,10) #w//6,h//6
                ...
                avg_age, avg_gender, avg_ethnicity=0,0,0
                for (x1,y1,x2,y2) in boxes[::-1]:
                    ...
                    t = time.time()
                    if age_gender_one_model:
                        age,gender,ethnicity,features=self.age_gender_fun(face_img)
                        elapsed = time.time() - t
                        if self.print_stat:
                    ...
                    avg_age+=age
                    avg_gender+=gender
                    avg_ethnicity+=ethnicity
                age=avg_age/len(boxes)
                gender=avg_gender/len(boxes)
                ethnicity=avg_ethnicity/len(boxes)
                ages.append(age)
        ...
        return bboxes, points,ages,genders,ethnicities,facial_features
    
    def show_detection_results(self,draw):
        bounding_boxes, points,ages,genders,ethnicities,facial_features=self.process_image(draw)
        
        for i,b in enumerate(bounding_boxes):
            b=[int(bi) for bi in b]
            x1,y1,x2,y2=b[0:4]
            
            age_ethnicity='%.0f %s'%(ages[i],FacialImageProcessing.get_ethnicity(ethnicities[i]))
            ...
            cv2.putText(draw,age_ethnicity, (x1, y1+10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0))
...
def get_image_file_properties(filepath, properties):
    res={}
    try:
        cmd = os.path.join(os.path.dirname(os.path.abspath(__file__)),'exiftool')+' -t -s -'+ ' -'.join(properties)+' '+filepath
        p = subprocess.Popen(
            cmd.split(' '),
        ...
        for prop in properties:
            reo_prop = re.compile(prop+'\s(?P<val>.*)', re.IGNORECASE)
            match_prop = reo_prop.search(str)
            if match_prop is not None:
                prop_val = match_prop.groups()[0]
                res[prop]=prop_val.rstrip()
...
def get_image_file_info(filepath):    
    if True:
        properties=['Model','FocalLength']
        img_props=get_image_file_properties(filepath, properties)
        return img_props.get(properties[0],''),re.sub('[^0-9.]+', '', img_props.get(properties[1],''))
    else:
        properties=['Model','ImageWidth','ImageHeight']
        img_props=get_image_file_properties(filepath, properties)
        return img_props.get(properties[0],''),int(img_props.get(properties[1],''))*int(img_props.get(properties[2],''))
...
def process_all_images(args, save_processed=False):
    ...
    for filename in args:
        ...
        #    draw=cv2.resize(draw, (min(width,640),min(height,480)))
        if width>height:
            draw=cv2.resize(draw, (800,600))
        else:
            draw=cv2.resize(draw, (600,800))
        draw,_=imgProcessing.show_detection_results(draw)
        #draw=cv2.resize(draw, (600,800))
