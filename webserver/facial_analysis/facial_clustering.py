...
try:
    from facial_analysis.facial_analysis import FacialImageProcessing,is_image
except:
...
use_clustering=sklearn_clustering 
...
def get_facial_clusters(dist_matrix,distanceThreshold=1,all_indices=None,no_images_in_cluster=1):    
    ...
    if use_clustering==rankorder_clustering:
        ...
        #db = hdbscan.HDBSCAN(metric='precomputed',min_cluster_size=no_images_in_cluster).fit(dist_matrix)
        #db = hdbscan.RobustSingleLinkage(metric='precomputed',cut=distanceThreshold, k=no_images_in_cluster-1,algorithm='generic').fit(dist_matrix)
        ...
        if True: #all_indices is None:
            ...
            for lbl in set(labels):
                ...
                if len(cluster)>1:
                    ...
                    penalties=np.array([[inf_dist*(all_indices[i]==all_indices[j] and i!=j) for j in cluster] for i in cluster])
                    if penalties.max()>0:
                        print('max penalties:',penalties.max())
                        single_photos=set()
                        for i in cluster:
                            for j in cluster:
                                if (all_indices[i]==all_indices[j] and i!=j):
                                    single_photos.add(all_indices[i])
                        print(single_photos)
                    ...
                    z = hac.linkage(condensed_dist_matrix, method='complete')
                    labels_cluster = hac.fcluster(z, inf_dist/2, 'distance')
                    new_clusters=[[cluster[ind] for ind,label in enumerate(labels_cluster) if label==l] for l in set(labels_cluster)]
                    print('orig:',len(cluster),'new:',sum([len(c) for c in new_clusters]), ' no clusters',len(new_clusters))
